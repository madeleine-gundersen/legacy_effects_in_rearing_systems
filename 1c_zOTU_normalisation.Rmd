...
all_dataset %>% filter(Order != "OrderInf") %>%
  ...
  scale_shape_manual("Overall_system",values = system_overall_shape21) +  
...
```{r}
all_dataset %>% filter(Order != "OrderInf") %>%
  ggplot2::ggplot(aes(x = Diversity_round, y = Diversity_rarefied)) + 
  my.theme + 
  scale_color_manual("Overall_system",  values = system_overall_color) + 
  scale_fill_manual("Overall_system",values = system_overall_fill) + 
  scale_shape_manual("Overall_system",values = system_overall_shape21) +  
  ...
  ggpubr::stat_cor(method = "pearson", label.y.npc = 0.9, 
                   aes(label = paste(..rr.label..)))  + 
  labs(title = "Rounded vs rarified correlation" ) 
```
...
wu_pro = procrustes.df_wu %>%    
  ...
  geom_point(aes(x=ax.1, y=ax.2, colour=Overall_system, shape = Overall_system, fill = Overall_system), size = 3) +
  geom_point(aes(x=ax.1, y=ax.2, shape = Overall_system), size = 3.5, fill= NA, color = "black") +
  ...
  labs(title = "Weigted UniFrac",
       subtitle = paste0("Procrustes sum of squares: ", round(procrustes.test_wu$ss, 4) ))+ 
...
u_pro = procrustes.df_u %>%   
  ...
  labs(title = "Unweighted UniFrac",
       subtitle = paste0("Procrustes sum of squares: ", round(procrustes.test_u$ss, 4) )) + 
  theme(text = element_text(size = 12))
```
...
bray_pro = procrustes.df_bray %>%   
  ...
  labs(title = "Bray Curtis",
       subtitle = paste0("Procrustes sum of squares: ", round(procrustes.test_bray$ss, 4) ))+ 
  theme(text = element_text(size = 12))
```
...
sor_pro = procrustes.df_sor %>%  
  ...
  xlab("Axis 1") + 
  labs(title = "Sørensen",
       subtitle = paste0("Procrustes sum of squares: ", round(procrustes.test_sor$ss, 4) )) + 
  theme(text = element_text(size = 12))
```
